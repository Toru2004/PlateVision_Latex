...
Đối với trường hợp \textit{nhận diện xe và logo} tại cổng ô tô, mô-đun ML sử dụng mô hình phát hiện đối tượng để xác định vùng mặt trước phương tiện và vị trí logo hãng xe. Các vùng ảnh này sau đó được đưa vào mô hình mạng nơ-ron Siamese đã được huấn luyện để trích xuất và so khớp đặc trưng hình ảnh của phương tiện. 
...
\node[block, below=0.5cm of iot] (notify) {Mobile App sinh viên cập nhâp trạng};

\node[block, below=0.5cm of notify] (guard) {Mobile App bảo vệ và \\ Web Dashboard quan sát};
...
% Nodes
\node[startstop] (start) {Bắt đầu};
\node[block, below=0.5cm of start] (prepare) {Sinh viên ấn nút \\ "Chuẩn bị rời đi" trên app};
...
\node[decision, below=0.5cm of exist] (ready) {Có yêu cầu \\ "rời đi" và còn lượt?};

\node[block, below=0.5cm of ready] (update) {Xóa xe khỏi Realtime DB, \\ cập nhật vào Firestore và trừ lượt};
...
\node[block, right=2cm of ready] (alert2) {Cảnh báo tức thì \\ đến Mobile App sinh viên};

\node[block, below=0.5cm of alert2] (guardnotif) {Thông báo đến \\ bảo vệ và admin};
...
Mô hình học máy trong hệ thống được đề xuất nhằm tự động hóa việc nhận diện và xác thực phương tiện tại trạm ra/vào bãi đỗ xe. Hệ thống tích hợp nhiều mô hình học sâu chuyên biệt để xử lý các loại phương tiện khác nhau, đảm bảo độ chính xác cao và khả năng vận hành ổn định trong môi trường thực tế.
...
\textbf{Nhận diện ô tô} được thực hiện thông qua mô hình mạng nơ-ron Siamese nhằm xác thực phương tiện dựa trên đặc trưng hình ảnh. Trong quá trình vận hành thực tế, hệ thống sử dụng YOLOv8 để phát hiện và định vị vùng xe ô tô trong khung hình, sau đó cắt vùng ảnh này ra để làm đầu vào cho mô hình Siamese. Việc cắt ảnh theo vùng xe giúp loại bỏ nền và các yếu tố nhiễu, cho phép mô hình Siamese tập trung vào đặc trưng của phương tiện và đạt hiệu suất tốt hơn.

Mô hình Siamese được huấn luyện để học cách so sánh hai ảnh xe và xác định chúng có phải cùng một phương tiện hay không. Trong giai đoạn huấn luyện, mô hình học trực tiếp từ ảnh xe đầy đủ mà không cần bước phát hiện và cắt ảnh. Khi triển khai thực tế, mô hình chiều từ ảnh xe đã cắt, sau đó so sánh vector này với các vector đặc trưng đã lưu trong Firestore khi người dùng đăng ký xe. Kết quả so sánh cho ra điểm tương đồng (similarity score), hệ thống sử dụng ngưỡng để quyết định xe có khớp hay không.
